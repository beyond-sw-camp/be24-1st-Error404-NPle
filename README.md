...
이커머스 플랫폼의 특성상 압도적으로 많은 조회 트래픽을 슬레이브 서버로 분산하여 마스터 서버의 부하를 줄이고 전체적인 응답 속도를 높이기 위한 구조입니다. HAProxy를 통해 트래픽을 유연하게 배분함으로써 특정 서버에 장애가 발생해도 서비스가 유지되는 고가용성을 확보할 수 있습니다. 또한  특정 시간에 공동구매가 오픈되면 짧은 순간 접속자가 급증하는데, 이때 모든 사용자가 상품 상세 정보와 옵션을 조회하는 SELECT 요청을 보내게 됩니다. 이러한 상황에서 슬레이브 서버만 추가하여 유연하게 대응할 수 있다는 장점이 있습니다.
---
